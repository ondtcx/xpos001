@php
    ...
    $posSidebarInitial = [
        'creditActive' => (bool) $oldAllowCreditSale,
        'paymentMethod' => $oldPaymentMethod,
        'selectedCustomerId' => $oldCustomerId ? (int) $oldCustomerId : null,
        'selectedCustomerName' => $oldCustomerId ? ($customers->firstWhere('id', (int) $oldCustomerId)?->name ?? '') : '',
        'fiadoAutoEnabled' => (bool) $fiadoAutoEnabled,
    ];
@endphp
...
<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-7xl space-y-4 sm:px-6 lg:px-8">
            <script>
                window.__POS_INITIAL__ = @json($posSidebarInitial);
            </script>
            @if (session('status'))
                        ...
                        <section class="rounded-lg border border-gray-200 bg-slate-50 p-4">
                            ...
                            <div class="mt-4 flex flex-wrap gap-2">
                                <button type="button" @click="$store.posSidebar.togglePanel('customer')" :class="$store.posSidebar.isButtonActive('customer') ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-gray-300 bg-white text-gray-700'" class="rounded-md border px-3 py-2 text-sm font-medium">
                                    Asignar cliente
                                    <span @click.stop="$store.posSidebar.togglePin('customer')" x-show="$store.posSidebar.isPanelVisible('customer')" class="ml-1.5 inline-flex items-center">
                                ...
                                <button type="button" @click="$store.posSidebar.togglePanel('payment')" :class="$store.posSidebar.isButtonActive('payment') ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-gray-300 bg-white text-gray-700'" class="rounded-md border px-3 py-2 text-sm font-medium">
                                    Cambiar método
                                    <span @click.stop="$store.posSidebar.togglePin('payment')" x-show="$store.posSidebar.isPanelVisible('payment')" class="ml-1.5 inline-flex items-center">
                                ...
                                <button type="button" @click="$store.posSidebar.togglePanel('received')" x-show="$store.posSidebar.paymentMethod === 'cash'" :class="$store.posSidebar.isButtonActive('received') ? 'border-indigo-300 bg-indigo-50 text-indigo-700' : 'border-gray-300 bg-white text-gray-700'" class="rounded-md border px-3 py-2 text-sm font-medium">
                                    ...
                                    <span @click.stop="$store.posSidebar.togglePin('received')" x-show="$store.posSidebar.isPanelVisible('received')" class="ml-1.5 inline-flex items-center">
                            ...
                            <div id="pos-customer-panel" x-show="$store.posSidebar.isPanelVisible('customer')" class="mt-4 rounded-lg border border-gray-200 bg-white p-3">
                            ...
                            <div id="pos-payment-methods-panel" x-show="$store.posSidebar.isPanelVisible('payment')" class="mt-4 rounded-lg border border-gray-200 bg-white p-3">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            cancelCreditSale?.addEventListener('click', () => {
                ...
                if (alpineData) {
                    alpineData.creditActive = false;
                    alpineData.activePanel = alpineData.activePanel === 'credit' ? null : alpineData.activePanel;
                    alpineData.syncToHiddenInputs();
                }
            });
