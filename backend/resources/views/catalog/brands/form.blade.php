<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
            <form method="POST" action="{{ $brand->exists ? route('brands.update', $brand) : route('brands.store') }}" class="space-y-6 rounded-lg bg-white p-6 shadow-sm ring-1 ring-catalog-border">
                @csrf
                @if ($brand->exists)
                    @method('PUT')
                @endif
